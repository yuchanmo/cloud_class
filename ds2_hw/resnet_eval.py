...
def evaluate(hps):
  ...
  for i in range(len(ckpt_state.all_model_checkpoint_paths)):
    ...
    #################################################
    ## Add summary of precision and best precision ##
    #################################################


    precision_summary = tf.Summary(value=[tf.Summary.Value(tag='precision', simple_value=precision)])
    summary_writer.add_summary(precision_summary, global_step=train_step)
    bestprecision_summary = tf.Summary(value=[tf.Summary.Value(tag='best_precision', simple_value=best_precision)])
    summary_writer.add_summary(bestprecision_summary, global_step=train_step)
